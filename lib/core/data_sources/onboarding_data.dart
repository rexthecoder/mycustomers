import 'dart:ui';

import 'package:mycustomers/core/models/onboarding_model.dart';


List<OnboardingModel> onboardData  = [
    OnboardingModel(
      Color(0xFFE6E6E6),
      Color(0xFF005699),
      Color(0xFFFFE074),
      2,
      'images/onboarding/flutter_onboarding_1.png',
      'EXPLORING',
      'FLUTTER',
      'On this channel we try and build fun stuff using Flutter and Dart. Subscribe for more content and let me know what you think in the comments!',
    ),
    OnboardingModel(
      Color(0xFF005699),
      Color(0xFFFFE074),
      Color(0xFF39393A),
      2,
      'images/onboarding/flutter_onboarding_2.png',
      'ANIMATING',
      'FLARE',
      'Making Flare animations and incorporating it into Flutter. Mobile animations have never been so easy!',
    ),
OnboardingModel(
      Color(0xFFFFE074),
      Color(0xFF39393A),
      Color(0xFFE6E6E6),
      0,
      'images/onboarding/flutter_onboarding_3.png',
      'DESIGN AND',
      'ANIMATION',
      'Recreating and imagining our own designs and animations to create custom effects and widgets.',
    ),
    OnboardingModel(
      Color(0xFF39393A),
      Color(0xFFE6E6E6),
      Color(0xFF005699),
      1,
      'images/onboarding/flutter_onboarding_4.jpg',
      'EDUCATIONAL',
      'PROGRAMMING',
      'Learn coding with examples, in a fun, somewhat structured, way. Enjoy!',
    ),
  ];