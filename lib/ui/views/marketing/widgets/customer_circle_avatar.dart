...
extension Initial on Contact {
  String get initial => (this == null)
      ? '.'
      : this.initials().isNotEmpty
          ? this.initials()
  ...
  Widget build(BuildContext context) {
    return CircleAvatar(
      child: this.child != null
          ? this.child
          : this.contact != null && this.contact.avatar.isNotEmpty
              ? Image.memory(
                  this.contact.avatar,
                  fit: BoxFit.cover,
                )
              : Text(
                  customer != null
                      ? '${customer.displayName.split(" ").length > 2 ? customer.displayName.split(" ")[0][0] ?? "" : customer.displayName[0]}'
                      : '${ccustomer.initials}',
